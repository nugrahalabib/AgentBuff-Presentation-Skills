...
`agentbuff-presentation` skill without making them the default for every deck.
...
## How To Use In AgentBuff Presentation
...
If the wildcard is custom, it must follow AgentBuff Presentation' no-slop aesthetics:
...
- Keep `agentbuff-presentation` output as one self-contained HTML file.
